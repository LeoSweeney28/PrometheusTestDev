...
return function(self, statement, funcDepth)
    ...
    self:addStatement(Ast.AssignmentStatement({
        ...
        Ast.OrExpression(Ast.AndExpression(self:register(scope, exprregs[3]), self:blockIdExpression(scope, bodyBlock.id)), self:blockIdExpression(scope, finalBlock.id))
